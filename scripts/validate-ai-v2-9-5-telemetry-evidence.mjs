...
for (const path of files) {
  ...
  if (!evidence || evidence.version !== VERSION || evidence.observerActive !== true) {
    errors.push(`${label}: missing active V2.9.5 opening-balance evidence`);
  } else {
    const targeted = Number(evidence.totalTargetedDecisions) || 0;
    const v295 = Number(evidence.totalV295Decisions) || 0;
    ...
    totals.v295Decisions += v295;
    totals.adjustedDecisions += adjusted;
    totals.fallbackDecisions += fallback;
    totals.publicInformationFailures += publicFailures;
    if (v295 !== targeted) errors.push(`${label}: V2.9.5 coverage ${v295}/${targeted}`);
    if (fallback) errors.push(`${label}: V2.9.5 fallback decisions ${fallback}`);
    if (publicFailures) errors.push(`${label}: V2.9.5 public-information failures ${publicFailures}`);
    for (const name of TARGETS) addRole(totals.byRole[name], evidence.roles?.[name]);
  }

  if (!integrity || integrity.version !== INTEGRITY_VERSION) {
...
if (totals.targetedDecisions <= 0) errors.push("no V2.9.5 targeted decisions were observed");
if (totals.v295Decisions !== totals.targetedDecisions) {
  errors.push(`aggregate V2.9.5 coverage ${totals.v295Decisions}/${totals.targetedDecisions}`);
}
if (totals.fallbackDecisions) errors.push(`aggregate V2.9.5 fallback decisions ${totals.fallbackDecisions}`);
if (totals.publicInformationFailures) {
  errors.push(`aggregate V2.9.5 public-information failures ${totals.publicInformationFailures}`);
}
...
if (!existsSync(aggregatePath)) {
  ...
  if (aggregate.telemetryRevision !== INTEGRITY_VERSION) {
  ...
  if (fullEvidence) {
    if (totals.adjustedDecisions <= 0) {
      errors.push("full run produced no V2.9.5 balance adjustments");
    }
    for (const name of TARGETS) {
      const evidence = totals.byRole[name];
      if (evidence.v295Decisions <= 0) errors.push(`${name}: no V2.9.5 decisions observed`);
      if (evidence.adjustedDecisions <= 0) errors.push(`${name}: no V2.9.5 adjustment observed`);
...
writeFileSync(
  join(outputDirectory, "ai-v2-9-5-strategy-evidence.json"),
  `${JSON.stringify(totals, null, 2)}\n`,
  "utf8",
);

console.log("# AI V2.9.5 opening balance and WTSD recovery");
console.log(`- Shards: ${totals.shards}`);
console.log(`- Targeted decisions: ${totals.targetedDecisions}`);
console.log(`- V2.9.5 decisions: ${totals.v295Decisions}`);
...
for (const name of TARGETS) {
  ...
  console.log(`- ${name}: ${role.v295Decisions} decisions; ${role.adjustedDecisions} adjusted${metric ? `; VPIP ${percent(metric.vpip)}; PFR ${percent(metric.pfr)}; WTSD ${percent(metric.wtsd)}` : ""}`);
}
