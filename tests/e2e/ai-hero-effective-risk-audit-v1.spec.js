...
const ENABLED = process.env.AI_HERO_EFFECTIVE_RISK_AUDIT === "1";
const requestedHands = Number.parseInt(process.env.AI_HERO_EFFECTIVE_RISK_HANDS || "250", 10);
const HANDS = Number.isInteger(requestedHands) ? Math.min(1_000, Math.max(1, requestedHands)) : 250;
const PAIR_INDEX = Math.max(0, Number.parseInt(process.env.AI_HERO_EFFECTIVE_RISK_PAIR || "0", 10) || 0);
const PROFILE = String(process.env.AI_HERO_EFFECTIVE_RISK_PROFILE || "balanced");
const BASE_SEED = Number.parseInt(process.env.AI_HERO_EFFECTIVE_RISK_SEED_BASE || "26893246", 10) || 26893246;
const OUTPUT_DIRECTORY = resolve(process.env.AI_HERO_EFFECTIVE_RISK_OUTPUT_DIR || "ai-hero-effective-risk-results");
...
const AUDIT_SCRIPT = resolve("tests/support/ai-hero-effective-risk-audit-v1.js");
...
function injectAuditHooks(source) {
  const replacements = [
    ...
    },
    {
      ...
      replacement: "        if (state.handOver && finalValidation.totalChips === expectedChips) {\n          window.AiHeroEffectiveRiskAuditV1?.recordHand?.(state.players, bigBlind, state.handNumber, heroProfile);\n          handsCompleted += 1;\n        }",
    },
    {
  ...
  for (const { target, replacement } of replacements) {
    ...
    if (matches !== 1) throw new Error(`Effective-risk hook target count ${matches}: ${target.slice(0, 80)}`);
    output = output.replace(target, replacement);
  }
  return output;
}

test.describe("AI Hero effective-risk audit V1", () => {
  test.skip(!ENABLED, "Run with AI_HERO_EFFECTIVE_RISK_AUDIT=1");
  ...
  test(`audits ${safeProfile(PROFILE)} pair ${PAIR_INDEX} for ${HANDS} hands`, async ({ page }) => {
    ...
    await expect.poll(() => page.evaluate(() => window.AiHeroEffectiveRiskAuditV1?.version || ""), { timeout: 10_000 }).toBe("1.0.0");
    ...
    await page.evaluate(() => {
      ...
      window.AiHeroEffectiveRiskAuditV1?.reset?.();
    });
    ...
    report.heroEffectiveRiskAudit = await page.evaluate(() => window.AiHeroEffectiveRiskAuditV1?.snapshot?.() || null);
    report.telemetryIntegrity = await page.evaluate(() => window.AiLongRunTelemetryIntegrityV294?.snapshot?.({ finalize: true }) || null);

    await mkdir(OUTPUT_DIRECTORY, { recursive: true });
    const outputPath = resolve(OUTPUT_DIRECTORY, `ai-hero-effective-risk-${profile}-pair-${PAIR_INDEX}.json`);
    ...
    expect(report.heroEffectiveRiskAudit.handsCompleted).toBe(HANDS);
