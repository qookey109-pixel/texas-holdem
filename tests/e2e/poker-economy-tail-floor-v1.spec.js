...
function injectTelemetryHooks(source) {
  const replacements = [
    { target: "    function recordAction(player, action) {\n      const metric = metricFor(player);", replacement: "    function recordAction(player, action) {\n      window.AiLongRunTelemetryIntegrityV294?.recordAction?.(player, action);\n      const metric = metricFor(player);" },
    { target: "        activeHand = createHandState(state.players, bigBlind, heroProfile);\n        const expectedChips", replacement: "        activeHand = createHandState(state.players, bigBlind, heroProfile);\n        window.AiLongRunTelemetryIntegrityV294?.beginHand?.();\n        const expectedChips" },
    { target: "        if (state.handOver && finalValidation.totalChips === expectedChips) handsCompleted += 1;", replacement: "        if (state.handOver && finalValidation.totalChips === expectedChips) {\n          window.PokerEconomyTailFloorV1?.recordHandSnapshot?.(state.players, bigBlind);\n          handsCompleted += 1;\n        }" },
    { target: "        cleanupTimers();\n        activeHand = null;", replacement: "        window.AiLongRunTelemetryIntegrityV294?.finalizeHand?.();\n        cleanupTimers();\n        activeHand = null;" },
  ];
  let output = source;
...
test.describe("Poker Economy Tail Floor Benchmark V1", () => {
  ...
  test(`evaluates ${POLICY} on shard ${SHARD_INDEX + 1}/${SHARD_COUNT} for ${HANDS} hands`, async ({ page }) => {
    ...
    expect(installed).toMatchObject({ installed: true, experimentOnly: true, productionBehaviorChanged: false, productionConfigUnchanged: true, publicInformationOnly: true, policy: { id: POLICY } });
    ...
    const report = await page.evaluate(options => window.AiLongRunTelemetryV29.runShard(options), { hands: HANDS, shardIndex: SHARD_INDEX, shardCount: SHARD_COUNT, baseSeed: BASE_SEED });
    ...
    expect(report.completedHands).toBe(HANDS);
    expect(report.failures).toEqual([]);
    expect(report.schedulerErrors).toEqual([]);
    expect(report.telemetryIntegrity).toMatchObject({ integrityPassed: true, errors: [] });
    expect(report.economyOoda).toMatchObject({ version: "1.0.0", schemaVersion: 3, experimentOnly: true, productionBehaviorChanged: false, productionConfigUnchanged: true, publicInformationOnly: true, hiddenOpponentCards: false, actualDeckOrder: false, futureBoardAnswer: false, predeterminedWinner: false, policy: { id: POLICY, maxBigBlinds: 60 }, handStackSampleCount: HANDS });
