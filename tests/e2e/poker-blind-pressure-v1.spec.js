...
const ENABLED = process.env.POKER_BLIND_PRESSURE === "1";
const POLICY = process.env.POKER_BLIND_PRESSURE_POLICY || "baseline";
const requestedHands = Number.parseInt(process.env.POKER_BLIND_PRESSURE_HANDS || "250", 10);
const HANDS = Number.isInteger(requestedHands) ? Math.min(1_000, Math.max(1, requestedHands)) : 250;
const SHARD_INDEX = Math.max(0, Number.parseInt(process.env.POKER_BLIND_PRESSURE_SHARD || "0", 10) || 0);
const SHARD_COUNT = Math.max(1, Number.parseInt(process.env.POKER_BLIND_PRESSURE_SHARD_COUNT || "1", 10) || 1);
const BASE_SEED = Number.parseInt(process.env.POKER_BLIND_PRESSURE_SEED_BASE || "26890246", 10) || 26890246;
const OUTPUT_DIRECTORY = resolve(process.env.POKER_BLIND_PRESSURE_OUTPUT_DIR || "blind-pressure-results");
...
const ADAPTER = resolve("tests/support/poker-blind-pressure-v1.js");
...
function injectTelemetryHooks(source) {
  const replacements = [
    ...
    },
    {
      ...
      replacement: "        if (state.handOver && finalValidation.totalChips === expectedChips) {\n          window.BlindPressureBenchmarkV1?.recordHandSnapshot?.(state.players, bigBlind);\n          handsCompleted += 1;\n        }",
    },
    {
  ...
  for (const { target, replacement } of replacements) {
    ...
    if (matches !== 1) throw new Error(`Blind-pressure hook target count ${matches}: ${target.slice(0, 72)}`);
    output = output.replace(target, replacement);
  }
  return output;
}

test.describe("Poker blind pressure benchmark V1", () => {
  test.skip(!ENABLED, "Run with POKER_BLIND_PRESSURE=1");
  ...
  test(`evaluates ${POLICY} on shard ${SHARD_INDEX + 1}/${SHARD_COUNT} for ${HANDS} hands`, async ({ page }) => {
    ...
    const installed = await page.evaluate(policy => window.BlindPressureBenchmarkV1.install(policy), POLICY);
    ...
    report.blindPressure = await page.evaluate(() => window.BlindPressureBenchmarkV1?.snapshot?.() || null);

    await mkdir(OUTPUT_DIRECTORY, { recursive: true });
    const suffix = padShard(SHARD_INDEX);
    const outputPath = resolve(OUTPUT_DIRECTORY, `poker-blind-pressure-${POLICY}-shard-${suffix}.json`);
    ...
    expect(report.blindPressure.handSamples).toHaveLength(HANDS);
    const maxBigBlind = Math.max(...report.blindPressure.handSamples.map(sample => Number(sample.bigBlind) || 0));
    if (POLICY === "baseline") expect(maxBigBlind).toBe(400);
    if (POLICY !== "baseline" && HANDS >= 150) expect(maxBigBlind).toBeGreaterThan(400);
