...
const ENABLED = process.env.AI_REPLACEMENT_CHURN_AUDIT === "1";
const requestedHands = Number.parseInt(process.env.AI_REPLACEMENT_CHURN_HANDS || "250", 10);
const HANDS = Number.isInteger(requestedHands) ? Math.min(1_000, Math.max(1, requestedHands)) : 250;
const SHARD_INDEX = Math.max(0, Number.parseInt(process.env.AI_REPLACEMENT_CHURN_SHARD || "0", 10) || 0);
const SHARD_COUNT = Math.max(1, Number.parseInt(process.env.AI_REPLACEMENT_CHURN_SHARD_COUNT || "1", 10) || 1);
const BASE_SEED = Number.parseInt(process.env.AI_REPLACEMENT_CHURN_SEED_BASE || "26890246", 10) || 26890246;
const OUTPUT_DIRECTORY = resolve(process.env.AI_REPLACEMENT_CHURN_OUTPUT_DIR || "ai-replacement-churn-results");
...
const AUDIT_SCRIPT = resolve("tests/support/ai-replacement-churn-audit-v1.js");
...
function injectAuditHooks(source) {
  ...
  for (const { target, replacement } of replacements) {
    ...
    if (matches !== 1) throw new Error(`Replacement-churn hook target count ${matches}: ${target.slice(0, 72)}`);
    output = output.replace(target, replacement);
  }
  return output;
}

test.describe("AI replacement churn runaway audit V1", () => {
  test.skip(!ENABLED, "Run with AI_REPLACEMENT_CHURN_AUDIT=1");
  test.setTimeout(TIMEOUT_MS);

  test(`audits replacement churn shard ${SHARD_INDEX + 1}/${SHARD_COUNT} for ${HANDS} hands`, async ({ page }) => {
    const pageErrors = [];
    page.on("pageerror", error => pageErrors.push(error.message));

    await page.goto("/", { waitUntil: "networkidle" });

    // Production AI layers install on native startup timers at 680/760/900ms.
    // Let those callbacks finish before the deterministic virtual scheduler takes over.
    ...
    await expect.poll(() => page.evaluate(() => window.AiReplacementChurnAuditV1?.version || ""), { timeout: 10_000 }).toBe("1.0.0");
    ...
    await page.evaluate(() => {
      ...
      window.AiReplacementChurnAuditV1?.reset?.();
    });
    ...
    report.replacementChurnAudit = await page.evaluate(() => window.AiReplacementChurnAuditV1?.snapshot?.() || null);
    report.telemetryIntegrity = await page.evaluate(() => window.AiLongRunTelemetryIntegrityV294?.snapshot?.({ finalize: true }) || null);

    await mkdir(OUTPUT_DIRECTORY, { recursive: true });
    const outputPath = resolve(OUTPUT_DIRECTORY, `ai-replacement-churn-audit-shard-${padShard(SHARD_INDEX)}.json`);
    ...
    expect(report.replacementChurnAudit.handsCompleted).toBe(HANDS);
