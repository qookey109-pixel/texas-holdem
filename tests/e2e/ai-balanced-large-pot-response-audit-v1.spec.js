...
const ENABLED = process.env.AI_BALANCED_LARGE_POT_RESPONSE_AUDIT === "1";
const requestedHands = Number.parseInt(process.env.AI_BALANCED_LARGE_POT_RESPONSE_HANDS || "250", 10);
const HANDS = Number.isInteger(requestedHands) ? Math.min(1_000, Math.max(1, requestedHands)) : 250;
const PAIR_INDEX = Math.max(0, Number.parseInt(process.env.AI_BALANCED_LARGE_POT_RESPONSE_PAIR || "0", 10) || 0);
const PROFILE = String(process.env.AI_BALANCED_LARGE_POT_RESPONSE_PROFILE || "balanced");
const BASE_SEED = Number.parseInt(process.env.AI_BALANCED_LARGE_POT_RESPONSE_SEED_BASE || "26891246", 10) || 26891246;
const OUTPUT_DIRECTORY = resolve(process.env.AI_BALANCED_LARGE_POT_RESPONSE_OUTPUT_DIR || "ai-balanced-large-pot-response-results");
...
const AUDIT_SCRIPT = resolve("tests/support/ai-balanced-large-pot-response-audit-v1.js");
...
function injectAuditHooks(source) {
  const replacements = [
    ...
    {
      target: "    window.logAction = function telemetryLogAction(player, action, amount = 0, note = \"\") {\n      recordAction(player, action);",
      replacement: "    window.logAction = function telemetryLogAction(player, action, amount = 0, note = \"\") {\n      window.AiBalancedLargePotResponseAuditV1?.recordAction?.(player, action, amount, {\n        street: streetKey(),\n        pot: Number(state?.pot) || 0,\n        bigBlind: Math.max(1, currentBigBlind()),\n        tier: String(window.AiTierStrategyV28?.targetRatings?.[player?.name]?.tier || \"\"),\n      });\n      recordAction(player, action);",
    },
    {
      target: "        activeHand = createHandState(state.players, bigBlind, heroProfile);\n        const expectedChips",
      replacement: "        activeHand = createHandState(state.players, bigBlind, heroProfile);\n        window.AiLongRunTelemetryIntegrityV294?.beginHand?.();\n        window.AiBalancedLargePotResponseAuditV1?.recordHandStart?.(state.players, bigBlind, state.handNumber, heroProfile);\n        const expectedChips",
    },
    {
      target: "        if (state.handOver && finalValidation.totalChips === expectedChips) handsCompleted += 1;",
      replacement: "        if (state.handOver && finalValidation.totalChips === expectedChips) {\n          window.AiBalancedLargePotResponseAuditV1?.recordHandEnd?.(state.players, bigBlind, state.handNumber, state.winners || [], state.board.length);\n          handsCompleted += 1;\n        }",
    },
    {
  ...
  for (const { target, replacement } of replacements) {
    ...
    if (matches !== 1) throw new Error(`Balanced large-pot response hook target count ${matches}: ${target.slice(0, 80)}`);
    output = output.replace(target, replacement);
  }
  return output;
}

test.describe("AI balanced large-pot response audit V1", () => {
  test.skip(!ENABLED, "Run with AI_BALANCED_LARGE_POT_RESPONSE_AUDIT=1");
  ...
  test(`audits ${safeProfile(PROFILE)} pair ${PAIR_INDEX} for ${HANDS} hands`, async ({ page }) => {
    ...
    await expect.poll(() => page.evaluate(() => window.AiBalancedLargePotResponseAuditV1?.version || ""), { timeout: 10_000 }).toBe("1.0.0");
    ...
    await page.evaluate(() => {
      ...
      window.AiBalancedLargePotResponseAuditV1?.reset?.();
    });
    ...
    report.balancedLargePotResponseAudit = await page.evaluate(() => window.AiBalancedLargePotResponseAuditV1?.snapshot?.() || null);
    report.telemetryIntegrity = await page.evaluate(() => window.AiLongRunTelemetryIntegrityV294?.snapshot?.({ finalize: true }) || null);

    await mkdir(OUTPUT_DIRECTORY, { recursive: true });
    const outputPath = resolve(OUTPUT_DIRECTORY, `ai-balanced-large-pot-response-${profile}-pair-${PAIR_INDEX}.json`);
    ...
    expect(report.balancedLargePotResponseAudit.handsCompleted).toBe(HANDS);
