# Long Session Gate 8 Manual Preview V2

> **Preview only. Do not merge or publish this branch.**
>
> Branch: `test/long-session-gate8-manual-preview-v2-20260815`
>
> Base: latest validated `main` `bd909d97524bba239067a82484b966a6460ec0e4`.
>
> Candidate source: PR #210 (`highest-affordable-fallback`).

## Why V2 exists

The original Gate 8 preview branch diverged from production after later merges. V2 is rebuilt directly from the current validated `main` and carries only the manual-preview runtime/visibility changes. It intentionally excludes the old PR #210 evidence runner/spec and does not alter production `main`.
...
- Desktop Responsive Layout V1 from PR #211 is included because this preview starts from current `main`.
...
```bash
git fetch origin
git switch --detach origin/test/long-session-gate8-manual-preview-v2-20260815
npm ci
