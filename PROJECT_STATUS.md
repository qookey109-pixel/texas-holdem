...
```text
9b69b297a98e4696257fac9dcec9cb1e3e31d2a9
```

此基準已包含 AI V2.7、300 次固定局面校準、模式 UI observer 穩定化、Production Smoke，以及每週 100 手 Poker State Stress。每次開始工作仍須重新讀取 GitHub `main`，不得把此 SHA 視為永久最新版本；合併後以新的 `main` commit 為準。
...
AI 僅可使用自己的底牌、公共牌、公開位置、公開下注行動、可見籌碼與聚合後的玩家統計。不得讀取對手隱藏底牌、實際牌堆順序、未來公共牌或預定勝負答案。
...
- 初始籌碼維持 `2,000`，初始盲注維持 `10 / 20`。
- 玩家與 AI 籌碼歸零後改用同一套重新買入公式。
- 目標為正籌碼牌桌平均的 `70%`。
- 上限為當前完整買入的 `60%` 與 `50BB` 中較低者。
- 下限為 `20BB`。

長期棄牌反制：

- 至少觀察 `8` 手。
- VPIP `<= 18%` 或翻牌前棄牌率 `>= 70%` 才標記為偏緊被動。
- 初中高階角色依性格使用約 `2.2～2.4BB` 後位偷盲。
- 乾燥翻牌／轉牌可使用約 `33～40% pot` 小尺寸施壓。
- Oracle、Chronos 與 Gemini 不由通用壓力層接管，避免覆蓋既有 Boss 引擎。

載入順序：

- 棄牌反制層先接入既有 `botAction`。
- AI V2.7 再作為最外層診斷包裝。
- 此順序已納入自然牌局壓力測試，防止 wrapper cycle、遞迴、卡死與殘留計時器。
...
- Worker 會白名單清理 `tournamentObservation`，只保留 VPIP、分街／位置行動率、近期公開行動、重複 All-in 聚合資訊與公開攤牌分類計數。
- 未列入白名單的欄位、對手隱藏牌、牌堆順序、未來公共牌與原始攤牌牌張清單不會送入 Gemini。

### G1 淘汰賽經濟

- 19 位永久淘汰賽，6 位開局、13 位依序補位，Gemini 最後登場。
- 淘汰賽採純手數盲注，不因玩家籌碼領先而加速。
- 一般中高階補位仍依當前大盲、全桌總 BB 與角色階級動態計算。
- 全桌目標深度 `170BB`，動態反應 `15%`。
- 中階 `25／35／45BB`；高階 `30／40／50BB`；G1 基礎特殊 Boss `35／45／60BB`；Gemini `40／50／70BB`。
- 正式 13 位補位角色的 G1 理論最大累積注入仍為 `660 entry-BB`。
- 支援同一手多位淘汰依序計算，並防止同一角色重複消費補位索引。
- 玩家籌碼、勝率或籌碼王狀態不影響盲注，也不影響一般中高階補位。

有限 Boss 追趕例外：

- 只使用牌桌可見籌碼。
- 玩家至少是目前最大 AI 籌碼的 `1.8 倍`才啟動。
- Oracle／Chronos 使用 `40／55／75BB` 的最低／目標／最高範圍。
- Gemini 使用 `50／65／90BB` 的最低／目標／最高範圍。
- 只增加 Boss 入場籌碼，不扣除玩家籌碼，不修改底牌、牌堆、牌面或勝負結果。
...
驗證一般模式對稱重買入、Boss 追趕上下限、低 VPIP／高棄牌分類、小尺寸壓力、公平資訊邊界，以及 Gemini Worker 公開觀察白名單。
...
- 驗證盲注不得倒退、角色不得重複、Gemini 最後登場，G1 基礎補位累積不得超過 `660 entry-BB`。
...
- 各角色平均底池、最大投入與存活手數。
- 一般模式重買入平均 BB 與玩家／AI 重買入差異。
- Boss 追趕啟動率、實際入場 BB 與登場後存活手數。
- 低 VPIP 反制觸發率、偷盲成功率與玩家體感。
- 500～1,000 手長跑。
- 長時間 DOM、動畫與瀏覽器記憶體監測。
...
- PR #108：AI V2.7 校準與 Production Smoke 同步。
- PR #109：一般模式公平重買入、Boss 有限追趕、長期棄牌反制與 Gemini 公開觀察接線。
