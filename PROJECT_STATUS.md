...
- **每次開始工作仍須重新讀取** `main` 最新 SHA、目前 open PR 與 CI；上面的 SHA 只記錄本次整理起點，不可永久當成最新基準。
...
- `build-manifest.json` 的 diagnostics feature 命名仍帶有較早的 V2.7 語意；目前結構驗證是綠的，但後續若要讓診斷頁完整反映 V2.9.5，可另開小 PR 更新 manifest，不應混在本次文件瘦身裡。
