// Reversible decision-area clarity trial: community slots, action color separation, and coach margin context.
(() => {
  "use strict";
  ...
  function installStyles() {
    ...
    style.textContent = `
      #boardCards .card.is-community-slot {
        opacity: .48 !important;
        color: transparent !important;
        border: 2px dashed rgba(233,188,101,.46) !important;
        background-color: rgba(4,14,18,.2) !important;
        background-image: none !important;
        box-shadow:
          inset 0 0 0 1px rgba(255,255,255,.025),
          inset 0 0 18px rgba(112,216,201,.035) !important;
        filter: none !important;
      }
      #boardCards .card.is-community-slot::before,
      #boardCards .card.is-community-slot::after,
      #boardCards .card.is-community-slot > * {
        opacity: 0 !important;
      }
  ...
  function syncCommunitySlots() {
    const boardCount = Array.isArray(window.state?.board)
      ? window.state.board.length
      : (typeof state !== "undefined" && Array.isArray(state.board) ? state.board.length : 0);
    const cards = Array.from(document.querySelectorAll("#boardCards .card"));

    cards.forEach((card, index) => {
      const empty = index >= boardCount;
      card.classList.toggle("is-community-slot", empty);
      if (empty) {
        card.setAttribute("aria-hidden", "true");
        card.dataset.communitySlot = String(index + 1);
      } else {
        card.removeAttribute("aria-hidden");
        delete card.dataset.communitySlot;
      }
    });

    return cards.filter(card => card.classList.contains("is-community-slot")).length;
  }
  ...
  function percentageNearLabel(panel, labelPattern) {
    ...
    const container = labelElement.closest(".coach-card, .coach-stat, section, article, div") || labelElement.parentElement;
  ...
  function syncCoachMargin() {
    ...
    note.dataset.tone = explanation.tone;
    note.innerHTML = `<strong>${explanation.title}</strong><span>${explanation.text}</span>`;
  ...
  function sync() {
    installStyles();
    const emptySlots = syncCommunitySlots();
    const coachNoteVisible = syncCoachMargin();
    return { emptySlots, coachNoteVisible };
  }
  ...
  if (arena) observer.observe(arena, { childList: true, subtree: true });
