// Fair special-boss strategy V2: public history, range inference, Monte Carlo equity, and mixed EV decisions.
(() => {
  "use strict";

  if (window.FairSpecialBossStrategy?.version === "2.0.0") return;
  ...
  const FAIR_PROFILE_VALUES = Object.freeze({
    Oracle: Object.freeze({
      style: "Player Model",
      fairPlay: true,
  ...
  function evaluateActions(player) {
    ...
    const adjustedEquity = clamp(equity - Math.max(0, activeCount - 1) * 0.018, 0, 1);
    ...
    for (const raiseBy of raiseCandidates(player, needed)) {
      ...
      const calledExtra = Math.min(raiseBy, Math.max(0, Number(human()?.stack) || 0));
      ...
      if (texture.wetness > 0.72 && adjustedEquity < 0.48 && risk / pot > 0.8) ev -= pot * 0.08;
  ...
  function lockProperty(target, key, value) {
    try {
  ...
  function applyFairData() {
    ...
    for (const name of SPECIAL_NAMES) {
      ...
      if (!profile) continue;
      for (const [key, value] of Object.entries(FAIR_PROFILE_VALUES[name])) lockProperty(profile, key, value);
      lockProperty(AI_PROFILE_META, name, FAIR_META[name]);
      lockProperty(DIALOGUE_BANK, name, FAIR_DIALOGUE[name]);
    }
    return true;
  }

  function installProfileMetaOverride() {
    if (window.__fairSpecialBossMetaV2Installed || typeof aiProfileMeta !== "function") return;
    window.__fairSpecialBossMetaV2Installed = true;
  ...
  function patchProfilePanel() {
    ...
    if (styleTag && styleTag.textContent !== selected.style) styleTag.textContent = selected.style;
  }

  function installProfileRenderHook() {
    if (window.__fairSpecialBossProfileRenderV2Installed || typeof renderAiProfilePanel !== "function") return;
    window.__fairSpecialBossProfileRenderV2Installed = true;
    originalRenderAiProfilePanel = renderAiProfilePanel;
    renderAiProfilePanel = function renderFairSpecialBossProfile(...args) {
  ...
  function patchArrival(event) {
    const banner = event.target?.closest?.("#specialBossArrivalBanner");
    if (!banner) return;
    const name = banner.textContent.includes("Chronos") ? "Chronos" : (banner.textContent.includes("Oracle") ? "Oracle" : "");
    if (!name) return;
    const copy = banner.querySelector("div > span");
    const fairText = name === "Oracle"
      ? "公平七星策略：分析公開行動與玩家習慣，不讀取底牌或未來牌。"
      : "公平七星策略：使用範圍、EV 與混合策略，不讀取底牌或未來牌。";
    if (copy && copy.textContent !== fairText) copy.textContent = fairText;
    banner.setAttribute("aria-label", `${name} 特殊 BOSS：${fairText}`);
  }

  function installVisualOverrides() {
    if (!document.querySelector("#fairSpecialBossStylesV2")) {
      const style = document.createElement("style");
      style.id = "fairSpecialBossStylesV2";
      style.textContent = `
    ...
    if (!window.__fairSpecialBossArrivalV2Installed) {
      window.__fairSpecialBossArrivalV2Installed = true;
      document.addEventListener("animationstart", patchArrival, true);
    }
  }

  function installDecisionOverride() {
    if (window.__fairSpecialBossDecisionV2Installed || typeof botAction !== "function" || !window.AiTierBossSystem?.version) return false;
    window.__fairSpecialBossDecisionV2Installed = true;
