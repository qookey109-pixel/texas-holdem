// Fair special-boss strategy V3: public history, range inference, Monte Carlo equity, and mixed EV decisions.
(() => {
  "use strict";

  if (window.FairSpecialBossStrategy?.version === "3.0.0") return;

  const VERSION = "3.0.0";
  const SPECIAL_NAMES = ["Oracle", "Chronos"];
  const FORBIDDEN_INFORMATION = [
    "opponent-hole-cards",
    "deck-order",
    "future-board",
    "precomputed-result",
  ...
  let originalRenderAiProfilePanel = null;
  let installTimer = 0;
  let repairTimer = 0;
  ...
  function cloneMeta(name) {
    return { ...META[name], traits: [...META[name].traits] };
  }

  function cloneDialogue(name) {
    return Object.fromEntries(Object.entries(DIALOGUE[name]).map(([key, lines]) => [key, [...lines]]));
  }
  ...
  function applyFairData() {
    ...
    for (const name of SPECIAL_NAMES) {
      ...
      if (profile) Object.assign(profile, PROFILE_VALUES[name]);
      AI_PROFILE_META[name] = cloneMeta(name);
      DIALOGUE_BANK[name] = cloneDialogue(name);
    }

    if (window.AiTierBossSystem) {
      window.AiTierBossSystem.fairPlay = true;
      window.AiTierBossSystem.usesHiddenInformation = false;
      try { delete window.AiTierBossSystem.projectedBoard; } catch (error) {
        window.AiTierBossSystem.projectedBoard = undefined;
      }
    }
  ...
  function publicPlayerModel() {
    ...
    return {
      ...
      contributionRatio: clamp((Number(hero?.totalContribution) || 0) / pot, 0, 4),
      currentHand: {
  ...
  function heroRangeWeight(cards, model) {
    ...
    if (["raise", "allin"].includes(model.currentAction) || model.currentHand.raised || model.currentHand.allIn) {
      weight = 0.12 + quality * (0.76 - model.raiseRate * confidence * 0.22);
      weight += model.raiseRate * confidence * 0.2;
    } else if (model.currentAction === "call" || model.currentHand.called) {
      weight = 0.28 + quality * 0.5 + model.callRate * confidence * 0.14;
    } else if (model.currentAction === "check" || model.currentHand.checked) {
      weight = 0.55 + quality * 0.2;
    }

    if (model.vpipRate > 0.62) weight += (1 - quality) * 0.12 * confidence;
    if (model.allInRate > 0.18 && model.currentAction === "allin") weight += quality * 0.14;
  ...
  function drawHeroRangeHand(pool, model) {
    for (let attempt = 0; attempt < 7 && pool.length >= 2; attempt += 1) {
      ...
      if (attempt === 6 || Math.random() <= heroRangeWeight(candidate, model)) return candidate;
      pool.push(first, second);
    }
    return drawRandomHand(pool);
  }
  ...
  function fairEquity(player, { iterations } = {}) {
    ...
    const runCount = Math.max(24, Math.min(96, Number(iterations) || (player.name === "Chronos" ? 64 : 52)));
    let equityTotal = 0;
    ...
    for (let simulation = 0; simulation < runCount; simulation += 1) {
      ...
      const comparisons = sampledHands.map(cards => compareResults(
        ownResult,
        evaluateBestHand([...cards, ...futureBoard]),
      ));
      if (comparisons.every(value => value > 0)) equityTotal += 1;
      else if (comparisons.every(value => value >= 0)) {
        equityTotal += 1 / (comparisons.filter(value => value === 0).length + 1);
      }
    }
  ...
  function boardWetness() {
    const board = Array.isArray(state?.board) ? state.board : [];
    if (board.length < 3) return 0.2;
    ...
    return clamp(flushPressure * 0.52 + connectedness * 0.48, 0, 1);
  }
  ...
  function evaluateActions(player) {
    ...
    const adjustedEquity = clamp(equity - Math.max(0, activeOpponents - 1) * 0.018, 0, 1);
    const confidence = model.confidence;
    ...
    for (const raiseBy of raiseCandidates(player, needed)) {
      ...
      const heroStack = Math.max(0, Number((typeof human === "function" ? human() : state.players?.[0])?.stack) || 0);
      const calledExtra = Math.min(raiseBy, heroStack);
      const calledEv = adjustedEquity * (pot + risk + calledExtra) - (1 - adjustedEquity) * risk;
    ...
    return { actions, equity: adjustedEquity, model, potOdds, needed, pot };
  }

  function chooseAction(player, analysis) {
    ...
    if (player.name === "Chronos" && closeness <= 0.1) return Math.random() < 0.68 ? best : second;
    if (player.name === "Oracle" && closeness <= 0.055) return Math.random() < 0.86 ? best : second;
    return best;
  }
  ...
  function fairBossAction(player) {
    ...
    let choice = chooseAction(player, analysis);
  ...
  function installMetaOverride() {
    if (window.__fairSpecialBossMetaV3Installed || typeof aiProfileMeta !== "function") return;
    window.__fairSpecialBossMetaV3Installed = true;
    originalAiProfileMeta = aiProfileMeta;
    aiProfileMeta = function fairBossProfileMeta(player) {
      return SPECIAL_NAMES.includes(player?.name) ? META[player.name] : originalAiProfileMeta(player);
    };
  }

  function patchProfilePanel() {
    ...
    if (!panel || panel.hidden || !selected || !SPECIAL_NAMES.includes(selected.name)) return;
    const meta = META[selected.name];
    ...
    if (list) list.innerHTML = meta.traits.map(trait => `<li>${escapeHtml(trait)}</li>`).join("");
    const styleTag = panel.querySelector(".ai-profile-tags span:nth-child(2)");
    if (styleTag) styleTag.textContent = PROFILE_VALUES[selected.name].style;
  }

  function installProfileHook() {
    if (window.__fairSpecialBossProfileV3Installed || typeof renderAiProfilePanel !== "function") return;
    window.__fairSpecialBossProfileV3Installed = true;
    originalRenderAiProfilePanel = renderAiProfilePanel;
    renderAiProfilePanel = function renderFairBossProfile(...args) {
      const result = originalRenderAiProfilePanel.apply(this, args);
      requestAnimationFrame(patchProfilePanel);
      return result;
    };
  }

  function installStyles() {
    if (document.querySelector("#fairSpecialBossStylesV3")) return;
    const style = document.createElement("style");
    style.id = "fairSpecialBossStylesV3";
    style.textContent = `
  ...
  function installDecisionOverride() {
    if (window.__fairSpecialBossDecisionV3Installed || typeof botAction !== "function" || !window.AiTierBossSystem?.version) return false;
    window.__fairSpecialBossDecisionV3Installed = true;
    window.__fairSpecialBossDecisionInstalled = true;
    originalBotAction = botAction;
    botAction = function fairSpecialBossBotAction(player) {
